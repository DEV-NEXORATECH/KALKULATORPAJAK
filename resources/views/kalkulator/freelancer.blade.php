<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator Pajak Freelancer') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="freelancerCalculator()" class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Pendapatan per Project (Rp)</label>
                                <input type="number" x-model="form.pendapatan_per_project" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan pendapatan per project">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Project per Tahun</label>
                                <input type="number" x-model="form.jumlah_project_per_tahun" min="1" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="12">
                            </div>
                            <div>
                ...
                <div x-show="result" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-heading font-semibold text-lg text-slate-900">Hasil Perhitungan Freelancer</h3>
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Total Pendapatan per Tahun</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.total_pendapatan)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                            ...
                            <div class="flex justify-between py-3">
                                <span class="text-sm text-slate-500">PPh 21 per Project</span>
                                <span class="text-lg font-bold text-green-500" x-text="formatRp(result.pph21_per_project)"></span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function freelancerCalculator() {
            return {
                loading: false,
                form: { pendapatan_per_project: '', jumlah_project_per_tahun: 12, ptkp_status: 'TK0', npwp: true },
                result: null,
                calcId: null,
                error: null,
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.freelancer.calculate") }}', {
                        method: 'POST',

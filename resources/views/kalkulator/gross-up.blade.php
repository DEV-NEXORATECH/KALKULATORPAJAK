<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator Gross Up') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="grossUpCalculator()" class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Target Take Home Pay (Rp)</label>
                                <input type="number" x-model="form.target_take_home_pay" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan target THP">
                            </div>
                            <div>
                ...
                <div x-show="result" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-heading font-semibold text-lg text-slate-900">Hasil Gross Up</h3>
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Target Take Home Pay</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.target_thp_bulanan)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Gaji Bruto yang Diperlukan</span>
                                <span class="text-lg font-bold text-primary-600" x-text="formatRp(result.gaji_bruto_bulanan)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh 21</span>
                                <span class="text-sm font-semibold text-red-500" x-text="formatRp(result.pph21_bulanan)"></span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-base font-semibold text-slate-900">Total Biaya Perusahaan</span>
                                <span class="text-lg font-bold text-green-500" x-text="formatRp(result.total_biaya)"></span>
                            </div>
                        </div>

                        <div class="mt-6 bg-primary-50 rounded-lg p-4">
                            <div class="flex items-start gap-3">
                                <svg class="w-5 h-5 text-primary-600 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <p class="text-sm text-primary-700">Dengan metode gross up, perusahaan menanggung pajak karyawan sehingga karyawan menerima THP bersih sesuai target.</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function grossUpCalculator() {
            return {
                loading: false,
                form: { target_take_home_pay: '', ptkp_status: 'TK0', npwp: true },
                result: null,
                calcId: null,
                error: null,
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.gross-up.calculate") }}', {
                        method: 'POST',

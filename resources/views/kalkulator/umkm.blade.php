<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator PPh Final UMKM') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="umkmCalculator()" class="space-y-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" x-model="form.omzet_tahunan" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan omzet tahunan">
                            </div>
                            <div>
                ...
                <div x-show="result" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-heading font-semibold text-lg text-slate-900">Hasil Perhitungan UMKM</h3>
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                ...
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.omzet)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPN Terutang</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.ppn_terutang)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh Final</span>
                        ...
                        <div class="mt-6 bg-slate-50 rounded-lg p-4">
                            <p class="text-sm text-slate-600">Berdasarkan PP 23 Tahun 2018, UMKM dengan omzet tidak melebihi Rp4.800.000.000 per tahun dikenakan PPh Final sebesar 0,5% dari omzet.</p>
                        </div>
                    </div>
                </div>
        ...
        function umkmCalculator() {
            return {
                ...
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.umkm.calculate") }}', {
                        method: 'POST',

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator Pajak Badan') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="badanCalculator()" class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Omzet Tahunan (Rp)</label>
                                <input type="number" x-model="form.omset_tahunan" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan omzet tahunan">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Biaya Operasional (Rp)</label>
                                <input type="number" x-model="form.biaya_operasional" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan biaya operasional">
                            </div>
                            <div>
                ...
                <div x-show="result" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-heading font-semibold text-lg text-slate-900">Hasil Perhitungan Pajak Badan</h3>
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Omzet Tahunan</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.omset)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Biaya Operasional</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.biaya)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Penghasilan Neto (PKP)</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.pkp)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Tarif PPh Badan</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="result.tarif_badan"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh Badan</span>
                                <span class="text-lg font-bold text-primary-600" x-text="formatRp(result.pph_badan)"></span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-sm text-slate-500">Total Pajak</span>
                                <span class="text-lg font-bold text-green-500" x-text="formatRp(result.total_pajak)"></span>
                            </div>
                        </div>

                        <div class="mt-6 bg-slate-50 rounded-lg p-4">
                            <p class="text-sm text-slate-600">Berdasarkan UU HPP, tarif PPh Badan untuk tahun 2022 dan seterusnya adalah 22%. Wajib pajak dengan omzet di atas Rp50 miliar mendapatkan fasilitas pengurangan tarif 50% untuk PKP sampai dengan Rp4,8 miliar.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function badanCalculator() {
            return {
                loading: false,
                form: { omset_tahunan: '', biaya_operasional: '', npwp: true },
                result: null,
                calcId: null,
                error: null,
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.badan.calculate") }}', {
                        method: 'POST',

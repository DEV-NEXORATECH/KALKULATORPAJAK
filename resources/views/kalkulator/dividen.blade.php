<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator Pajak Dividen') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="dividenCalculator()" class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Dividen (Rp)</label>
                                <input type="number" x-model="form.jumlah_dividen" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan jumlah dividen">
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">WP Luar Negeri</label>
                                <div class="flex items-center gap-3 mt-2">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" x-model="form.is_wpln" class="sr-only peer">
                                        <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-600"></div>
                                        <span class="ml-3 text-sm text-slate-600" x-text="form.is_wpln ? 'WP Luar Negeri' : 'WP Dalam Negeri'"></span>
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <div x-show="result" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-heading font-semibold text-lg text-slate-900">Hasil Perhitungan Dividen</h3>
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Jumlah Dividen</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.jumlah_dividen)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">Tarif PPh</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="result.tarif + '%'"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh Dividen</span>
                                <span class="text-lg font-bold text-primary-600" x-text="formatRp(result.pph_dividen)"></span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-sm text-slate-500">Dividen Bersih</span>
                                <span class="text-lg font-bold text-green-500" x-text="formatRp(result.dividen_bersih)"></span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-sm text-slate-500">Keterangan</span>
                                <span class="text-sm text-slate-600" x-text="result.keterangan"></span>
                            </div>
                        </div>

                        <div class="mt-6 bg-slate-50 rounded-lg p-4">
                            <p class="text-sm text-slate-600">Berdasarkan UU HPP, dividen yang diterima WP Dalam Negeri dapat dikecualikan dari objek pajak jika diinvestasikan di Indonesia dalam jangka waktu tertentu. WP Luar Negeri dikenakan PPh Pasal 26 sebesar 20%.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function dividenCalculator() {
            return {
                loading: false,
                form: { jumlah_dividen: '', npwp: true, is_wpln: false },
                result: null,
                calcId: null,
                error: null,
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.dividen.calculate") }}', {
                        method: 'POST',

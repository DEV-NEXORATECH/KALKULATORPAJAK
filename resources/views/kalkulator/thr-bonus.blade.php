<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Kalkulator THR / Bonus') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4">
            <div x-data="thrCalculator()" class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form x-on:submit.prevent="hitung">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">THR / Bonus (Rp)</label>
                                <input type="number" x-model="form.thr_bonus" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="Masukkan jumlah THR">
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Bulan Kerja</label>
                                <input type="number" x-model="form.jumlah_bulan_kerja" min="1" max="12" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm" placeholder="12">
                            </div>
                            <div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh 21 Normal (tanpa THR)</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.pph21_normal)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100">
                                <span class="text-sm text-slate-500">PPh 21 dengan THR</span>
                                <span class="text-sm font-semibold text-slate-900" x-text="formatRp(result.pph21_dengan_thr)"></span>
                            </div>
                            <div class="flex justify-between py-3 border-b border-slate-100 bg-red-50 rounded px-3">
                                <span class="text-sm font-medium text-red-700">Pajak THR</span>
                                <span class="text-sm font-bold text-red-600" x-text="formatRp(result.pph21_thr)"></span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-lg font-semibold text-slate-900">THR Bersih</span>
                                <span class="text-lg font-bold text-green-500" x-text="formatRp(result.thr_bersih)"></span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function thrCalculator() {
            return {
                loading: false,
                form: { gaji_bulanan: '', thr_bonus: '', ptkp_status: 'TK0', npwp: true, jumlah_bulan_kerja: 12 },
                result: null,
                calcId: null,
                error: null,
                hitung() {
                    this.loading = true;
                    this.error = null;
                    fetch('{{ route("kalkulator.thr-bonus.calculate") }}', {
                        method: 'POST',
